...
set_SetNotificationForWaitCompletion_flag(StackFrame *frame) {
	...
	gboolean arg = TRUE;
	args[0] = &arg;
	mono_runtime_invoke(setNotificationMethod, mono_object_unbox(builder), args, &exc);
	if (exc != NULL) {
		  ...
		  gboolean step_to_catch, StackFrame **frames, int nframes)
{
	...
	if (step_to_catch) {
		...
		if (asyncMethod && nframes && ensure_jit(frames[0])) {
			//asyncMethod has value, this means we are inside async method
