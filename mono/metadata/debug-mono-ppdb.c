...
gboolean is_async_method_stepping_information_guid(guint8* debugInfoKindGuid)
{
...
const char* lookup_custom_debug_information(MonoImage* image, guint32 token, uint8_t parent_type, GuidComparer comparer)
{
	MonoTableInfo *tables = image->tables;
	...
	for (int i = loc.result + 1; i < table->rows; i++)
	{
		if (mono_metadata_decode_row_col(table, i, MONO_CUSTOMDEBUGINFORMATION_PARENT) != loc.idx)
			break;
		if (comparer(mono_metadata_guid_heap(image, mono_metadata_decode_row_col(table, i, MONO_CUSTOMDEBUGINFORMATION_KIND)))) {
...
mono_ppdb_lookup_method_async_debug_info(MonoDebugMethodInfo *minfo)
{
	...
	while (pointer - blob < blob_len) {
		...
		while ((*pointer & 0x80) > 0)
			pointer++;//moveNextMethodToken when >127
		pointer++;//moveNextMethodToken
	}
